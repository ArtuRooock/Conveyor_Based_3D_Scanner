...
UNIT_SCALE  = 10.0               # множитель координат при чтении файла:
                                # 10.0 если данные в сантиметрах (калибровка
                                # по доске с размером клетки в см),
                                # 1.0 если уже в миллиметрах
DIRECTION   = [1.0, 0.0, 0.0]   # ось, вдоль которой развёрнуто облако
BELT_NORMAL = [0.0, 1.0, 0.0]   # ось высоты
BELT_LEVEL  = 1.19              # уровень ленты; None = оценить автоматом
BELT_EPS    = 4.0               # точки ниже (лента + eps) выбрасываются
SLICE_STEP  = 0.5               # толщина слоя ds
CELL        = 0.2               # пиксель растра площади
MIN_GAP     = 5.0               # минимальный зазор между предметами
CLOSED_BOTTOM = True            # плоское дно на ленте
PROFILE_MODE  = False            # одна камера сверху: площадь по огибающей
MIN_VOLUME  = 100.0               # фигуры мельче этого - мусор, не показывать
TARGET_VOLUME = 21334.0            # целевой объём порции для поиска точек реза;
                                # None = резка отключена
MIN_BIN_FRAC = 0.0             # доля от медианного заполнения слоя, ниже
...
W_MIN       = None              # поперечные границы рабочей зоны ленты:
W_MAX       = None              # точки вне (W_MIN..W_MAX) по оси поперёк
                                # ленты выбрасываются (отражения, фон);
                                # None = не ограничивать
VOXEL       = 0.5               # размер вокселя фильтра шума
MIN_VOXEL_PTS = 0              # порог точек в вокселе (ниже - шум);
...
def load_pts(path):
    """Читает облако из .pts.

    Поддерживаются два формата:
        индекс x y z  (4 колонки) - обычный, с номером среза
        x y z         (3 колонки) - без индекса; тогда индексы срезов
                      восстанавливаются по продольной координате, чтобы
                      алгоритм мог работать послойно
    Координаты умножаются на UNIT_SCALE (перевод см -> мм при необходимости).
    """
    data = np.loadtxt(path)
    if data.ndim != 2 or data.shape[1] not in (3, 4):
        raise ValueError("ожидался формат: 'индекс x y z' или 'x y z'")

    if data.shape[1] == 4:
        frames = data[:, 0].astype(int)
        pts = data[:, 1:4] * UNIT_SCALE
    else:
        pts = data * UNIT_SCALE
        # индекса среза в файле нет: нарезаем облако на условные срезы
        # по продольной координате, шагом в один слой.
        # ВАЖНО: нумеруем по УБЫВАНИЮ s - именно так срезы приходят с
        # реальной ленты (координата s движущегося предмета уменьшается),
        # и только в этом порядке алгоритм закрывает слои правильно
        u = np.asarray(DIRECTION, dtype=float)
        u /= np.linalg.norm(u)
        s = pts @ u
        frames = np.floor((s.max() - s) / SLICE_STEP).astype(int)
        print("индекс среза не найден: восстановлен по координате "
              "(получилось %d условных срезов)" % len(np.unique(frames)))
    return frames, pts
...
def diagnose(frames, pts, u, n):
    ...
    if abs(s1 - s0) > 1.0:
        ...
        if drift > 0:
            print("  !! ВНИМАНИЕ: координата s РАСТЁТ с номером среза.")
            print("     На реальной ленте она убывает. Объёмы получатся")
            print("     заниженными в разы. Поменяйте знак DIRECTION:")
            print("     DIRECTION = [%s]"
                  % ", ".join("%.1f" % (-c) for c in u))
    else:
